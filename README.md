...
The Netlify deploy proxies same-origin `/api/*` requests back to `https://api.antarctic.games`, so the browser shell does not have to reach the backend origin directly just to bootstrap web browsing.
